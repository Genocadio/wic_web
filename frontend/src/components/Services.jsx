...
const ServiceDisplay = ({ service, handleEdit, toggleShowImages, toggleShowVideos, deleteService }) => {
  const { Name, Description, Type, ImageLinks, VideoLinks, showImages, showVideos } = service;
          ...
          {editedService.VideoLinks &&
            editedService.VideoLinks.map((link, idx) => {
              ...
              if (link.includes('youtu.be') || link.includes('youtube.com')) {
                ...
                if (videoId) {
                  return <YouTubeVideo key={idx} videoId={videoId} />;
                } else {
                  return <p key={idx}>Invalid YouTube link: {link}</p>;
                }
              } else {
                return (
                  <video key={idx} src={link.trim()} controls style={{ maxWidth: '200px', margin: '5px' }}>
